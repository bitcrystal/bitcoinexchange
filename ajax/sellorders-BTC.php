...
if($count!=0) {
   $Query = mysql_query("SELECT amount, rate FROM sell_orderbook WHERE want='$my_coins->coins_names_prefix[0]' and processed='1' ORDER BY rate ASC");
   while($Row = mysql_fetch_assoc($Query)) {
      ...
      $buy_subtotal = "0";
      $Query = mysql_query("SELECT amount FROM sell_orderbook WHERE want='$my_coins->coins_names_prefix[0]' and processed='1' ORDER BY rate ASC");
      while($Row = mysql_fetch_assoc($Query)) {
      ...
      echo '<table>
               <tr>
                  ...
                  <td colspan="3" align="right" style="padding: 2px;" nowrap>Total '.$my_coins->coins_names_prefix[2].': '.$cncvalue.'</td>
               </tr><tr>
