...
$sql = "SELECT * FROM buy_orderbook WHERE want='$coins_names_prefix[0]' and processed='1'";
$result = mysql_query($sql);
$count = mysql_num_rows($result);
if($count!=0) {
   $Query = mysql_query("SELECT amount, rate FROM buy_orderbook WHERE want='$coins_names_prefix[0]' and processed='1' ORDER BY rate DESC");
   while($Row = mysql_fetch_assoc($Query)) {
      ...
      $buy_subtotal = "0";
      $Query = mysql_query("SELECT amount,rate FROM buy_orderbook WHERE want='$coins_names_prefix[0]' and processed='1' ORDER BY rate ASC");
      while($Row = mysql_fetch_assoc($Query)) {
   ...
   echo '<table>
         <tr>
            <td colspan="3" align="left" style="font-weight: bold; padding: 2px;" nowrap>Buy Orders</td>
                  ...
                  <td colspan="3" align="right" style="padding: 2px;" nowrap>Total '.$coins_names_prefix[0].': '.$btevalue.'</td>
               </tr><tr>
            <td align="left" style="font-weight: bold; padding: 1px;" nowrap>Amount ('.$coins_names_prefix[2].')</td>
            <td align="left" style="font-weight: bold; padding: 1px; padding-left: 10px;" nowrap>Rate ('.$coins_names_prefix[0].')</td>
            <td align="left" style="font-weight: bold; padding: 1px; padding-left: 10px;" nowrap>Total ('.$coins_names_prefix[0].')</td>
         </tr>'.$Buys_td.'</table>';
} else {
