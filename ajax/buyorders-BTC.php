...
if($count!=0) {
   $Query = mysql_query("SELECT amount, rate FROM buy_orderbook WHERE want='$my_coins->$coins_names_prefix[0]' and processed='1' ORDER BY rate DESC");
   while($Row = mysql_fetch_assoc($Query)) {
      ...
      $buy_subtotal = "0";
      $Query = mysql_query("SELECT amount,rate FROM buy_orderbook WHERE want='$my_coins->$coins_names_prefix[0]' and processed='1' ORDER BY rate ASC");
      while($Row = mysql_fetch_assoc($Query)) {
