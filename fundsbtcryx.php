   ...
   if($withdraw_address) {
      if($withdraw_amount) {
         ...
         if($withdraw_amount<=$Bitcrystalxd_Balance) {
            $set_withdraw_amount = $withdraw_amount - $coins[$coins_names[2]]["fee"]];       // minus the fee
            ...
            if($Bitcrystalxd_Withdraw_From) {
               $result = minusfunds($user_session,$coins_names_prefix[2],$withdraw_amount);
               $result = plusfunds($FEEBEE,$coins_names_prefix[2],$coins[$coins_names[2]]["fee"]);        // add fee to feebee account
               $Bitcrystalxd_Balance = userbalance($user_session,$coins_names_prefix[2]);
               ...
               if(!mysql_query("INSERT INTO transactions (id,date,username,action,coin,address,txid,amount) VALUES ('','$date','$user_session','withdraw','$coins_names_prefix[2]','$withdraw_address','$Bitcrystalxd_Withdraw_From','$withdraw_amount')")){
            ...
            $withdraw_message = 'You do not have enough '.$coins_names[2].'!';
         }
      } else {
         $withdraw_message = 'No amount to withdraw was entered!';
      }
   } else {
      $withdraw_message = 'No '.$coins_names[2].' address was entered!';
   }
}
?>
<html>
      ...
      <tr>
         ...
         <td align="center" valign="top" style="padding: 5px;" nowrap>
            <table>
               ...
               <tr><tr>
                  <?php echo '<td align="left" style="padding: 2px; padding-left: 20px;">Deposits must have 6 confirmations to become active. There is a fee of '.$coins[$coins_names[2]]["fee"].' '.$coins_names[2].'s to make a withraw.</td>'; ?>
               </tr>
            </table>
            </center>
         </td>
