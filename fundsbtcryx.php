   ...
   if($withdraw_address) {
      if($withdraw_amount) {
         ...
         if($withdraw_amount<=$Bitcrystalxd_Balance) {
            $set_withdraw_amount = $withdraw_amount - 0.00;       // minus the fee
            ...
            if($Bitcrystalxd_Withdraw_From) {
               $result = minusfunds($user_session,"BTCRYX",$withdraw_amount);
               //$result = plusfunds($FEEBEE,"BTCRYX","0.01");        // add fee to feebee account
               ...
               if(!mysql_query("INSERT INTO transactions (id,date,username,action,coin,address,txid,amount) VALUES ('','$date','$user_session','withdraw','BTCRYX','$withdraw_address','$Bitcrystalxd_Withdraw_From','$withdraw_amount')")){
            ...
            $withdraw_message = 'You do not have enough Bitcrystalx!';
         }
      } else {
      ...
      <tr>
         ...
         <td align="center" valign="top" style="padding: 5px;" nowrap>
            <table>
               ...
               <tr><tr>
                  <td align="left" style="padding: 2px; padding-left: 20px;">Deposits must have 6 confirmations to become active. There is a fee of 0.00 Bitcrystalx to make a withraw.</td>
               </tr>
            </table>
            </center>
         </td>
