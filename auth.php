...
if(!$user_session) {
   ...
   $Bitcoind_Balance = userbalance($user_session,"$coins_names_prefix[0]");
   ...
   foreach($Bitcoind_List_Transactions as $Bitcoind_List_Transaction) {
      if($Bitcoind_List_Transaction['category']=="receive") {
         if(6<=$Bitcoind_List_Transaction['confirmations']) {
            ...
            $DEPOSIT_coin_type = "$coins_names_prefix[0]";
            ...
            if($num_rows!=1) {
               if(!mysql_query("INSERT INTO transactions (id,date,username,action,coin,address,txid,amount) VALUES ('','$DEPOSIT_date','$user_session','$DEPOSIT_tx_type','$DEPOSIT_coin_type','$DEPOSIT_address','$DEPOSIT_txid','$DEPOSIT_amount')")) {
                  ...
                  $result = plusfunds($user_session,"$coins_names_prefix[0]",$DEPOSIT_amount);
                  if($result) {
                     $r_system_action = "success";
                  } else {
                     die("Server error");
   ...
   foreach($Bitcrystalxd_List_Transactions as $Bitcrystalxd_List_Transaction) {
      if($Bitcrystalxd_List_Transaction['category']=="receive") {
         if(6<=$Bitcrystald_List_Transaction['confirmations']) {
            $DEPOSIT_tx_type = 'deposit';
            $DEPOSIT_coin_type = "$coins_names_prefix[2]";
            ...
            if($num_rows!=1) {
               if(!mysql_query("INSERT INTO transactions (id,date,username,action,coin,address,txid,amount) VALUES ('','$DEPOSIT_date','$user_session','$DEPOSIT_tx_type','$DEPOSIT_coin_type','$DEPOSIT_address','$DEPOSIT_txid','$DEPOSIT_amount')")) {
                  ...
                  $result = plusfunds($user_session,"$coins_names_prefix[2]",$DEPOSIT_amount);
                  if($result) {
                     $r_system_action = "success";
                  } else {
                     die("Server error");
   ...
   foreach($Bitcrystald_List_Transactions as $Bitcrystald_List_Transaction) {
      if($Bitcrystald_List_Transaction['category']=="receive") {
         if(6<=$Bitcrystald_List_Transaction['confirmations']) {
            $DEPOSIT_tx_type = 'deposit';
            $DEPOSIT_coin_type = "$coins_names_prefix[1]";
            ...
            if($num_rows!=1) {
               if(!mysql_query("INSERT INTO transactions (id,date,username,action,coin,address,txid,amount) VALUES ('','$DEPOSIT_date','$user_session','$DEPOSIT_tx_type','$DEPOSIT_coin_type','$DEPOSIT_address','$DEPOSIT_txid','$DEPOSIT_amount')")) {
                  ...
                  $result = plusfunds($user_session,"$coins_names_prefix[1]",$DEPOSIT_amount);
                  if($result) {
                     $r_system_action = "success";
                  } else {
                     die("Server error");
   ...
   $Bitcoind_Balance = userbalance($user_session,"$coins_names_prefix[0]");      // Simple function to call the users balance
   $Bitcrystald_Balance = userbalance($user_session,"$coins_names_prefix[1]");
   $Bitcrystalxd_Balance = userbalance($user_session,"$coins_names_prefix[2]");
}
?>